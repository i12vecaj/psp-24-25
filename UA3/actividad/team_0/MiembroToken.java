...
public class MiembroToken implements Runnable {
    ...
    public void iniciar() {
        ...
        try (DatagramSocket socket = new DatagramSocket(puerto)) {
            ...
            while (true) {
                if (!tengoToken) {            //cuando tengoToken es True, ejecuta una tarea simulada con sleep() en el metodo run.
    ...
    private void enviarToken() {
        try (DatagramSocket socket = new DatagramSocket()) {
            byte[] buffer = new byte[1];                                    //cuando termina la tarea, envía el token al siguiente miembro con UDP, usando DatagramSocket y DatagramPacket.
